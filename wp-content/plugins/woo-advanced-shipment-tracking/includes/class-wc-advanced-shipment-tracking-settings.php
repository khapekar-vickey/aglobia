...
class WC_Advanced_Shipment_Tracking_Settings {		
	...
	*/
	...
	public function ast_open_inline_tracking_form_fun(){
		...
		$shippment_countries = $wpdb->get_results( $wpdb->prepare( "SELECT shipping_country FROM $woo_shippment_table_name WHERE display_in_order = 1 GROUP BY shipping_country" ) );
		
		$shippment_providers = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $woo_shippment_table_name" ) );
	...
	public function sync_providers_fun(){
		...
		if ( is_array( $resp ) && ! is_wp_error( $resp ) ) {
			...
			if($reset_checked == 1){
				...
				$status = 'active';
				$default_shippment_providers = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $this->table WHERE display_in_order = 1" ) );	
				ob_start();
				...
				$default_shippment_providers = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $this->table WHERE shipping_default = 1" ) );
...
add_filter( 'woocommerce_analytics_report_menu_items', 'woocommerce_analytics_report_menu_items_add', 10, 1 );

function woocommerce_analytics_report_menu_items_add($report_pages){
	$features = wc_admin_get_feature_config();
	$report_pages[] = array(
		'id'     => 'woocommerce-analytics-shipping-providers',
		'title'  => __( 'Shipping Providers', 'woocommerce' ),
		'parent' => 'woocommerce-analytics',
		'path'   => '/shipping_providers',
	);
	return $report_pages;
}