...
class Class_Admin_Network_Products_List_Table extends \WP_List_Table {
	...
	public function column_product_type( $item ) {
		...
		switch ( $product->get_type() ) {
			case 'grouped':
				$data = array(
					'class' => 'grouped',
					'name'  => __( 'Grouped', 'woonet' ),
				);
				break;
			case 'external':
				$data = array(
					'class' => 'external',
					'name'  => __( 'External/Affiliate', 'woonet' ),
				);
				break;
			case 'variable':
				$data = array(
					'class' => 'variable',
					'name'  => __( 'Variable', 'woonet' ),
				);
				break;
			case 'simple':
				if ( $product->is_virtual() ) {
					$data = array(
						'class' => 'virtual',
						'name'  => __( 'Virtual', 'woonet' ),
					);
				} elseif ( $product->is_downloadable() ) {
					$data = array(
						'class' => 'downloadable',
						'name'  => __( 'Downloadable', 'woonet' ),
					);
				} else {
					$data = array(
						'class' => 'simple',
						'name'  => __( 'Simple', 'woonet' ),
					);
				}
				break;
			default:
	...
	private function get_products( $args = array() ) {
		...
		if ( empty( $args['search'] ) ) {
			...
			$sub_query = "
				SELECT ID AS id, CAST( post_title AS CHAR CHARACTER SET utf8 ) AS name, post_date AS 'date', %1\$d AS blog_id
				FROM %2\$s AS posts
				JOIN %3\$s AS postmeta ON postmeta.post_id = posts.ID AND postmeta.meta_key = '_sku'
				WHERE post_type='product' ";
		}
		...
		foreach ( $blog_ids as $blog_id ) {
			...
			if ( ! empty( $args['search'] ) ) {
				$query[ $blog_id ] .= " AND (post_title LIKE '%{$args['search']}%' OR post_content LIKE '%{$args['search']}%' OR postmeta.meta_value LIKE '%{$args['search']}%')";
			}
		}
	...
	public function inline_edit() {
		...
		foreach ( $blog_ids as $blog_id ) {
			...
			$blog_categories[ $blog_id ] = wp_terms_checklist(
				null,
				array(
				...
				while ( $bulk < 2 ) {
					...
					" style="display: none"><td colspan="<?php echo $this->get_column_count(); ?>" class="colspanchange">
							...
							<fieldset class="inline-edit-col-right"><div class="inline-edit-col">
									...
									<?php
									if ( post_type_supports( 'product', 'comments' ) || post_type_supports( 'product', 'trackbacks' ) ) :
										if ( $bulk ) :
											?>
