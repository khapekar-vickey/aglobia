 ...
 * Sidebar position
 */
add_filter( 'theme_mod_sidebar_position', 'buildex_set_post_meta_value' );

/**
 * Container type
 */
add_filter( 'theme_mod_container_type', 'buildex_set_post_meta_value' );

/**
 * Header layout type
 */
add_filter( 'theme_mod_header_layout_type', 'buildex_set_header_layout_value' );
 ...
 * @return string
 */
function buildex_set_post_meta_value( $value ) {
	$queried_obj = buildex_get_queried_obj();

	if ( ! $queried_obj ) {
		return $value;
	}

	$meta_key   = 'buildex_' . str_replace( 'theme_mod_', '', current_filter() );
 ...
 * @return string
 */
function buildex_set_header_layout_value( $value ) {

	if ( wp_is_mobile() ) {
		return 'mobile';
	}

	return buildex_set_post_meta_value( $value );
}
 ...
 */
function buildex_get_queried_obj() {
	$queried_obj = apply_filters( 'buildex-theme/posts/queried_object_id', false );

	if ( ! $queried_obj && ! buildex_maybe_need_rewrite_mod() ) {
		return false;
	}
 ...
 * @return boolean
 */
function buildex_maybe_need_rewrite_mod() {
 ...
 * @return string
 */
function buildex_render_macros( $string ) {

	$macros = apply_filters( 'buildex-theme/data_macros', array(
		'/%%year%%/' => date( 'Y' ),
 ...
 * @return string
 */
function buildex_render_icons( $content ) {
	$icons     = buildex_get_render_icons_set();
	$icons_set = implode( '|', array_keys( $icons ) );

	$regex = '/icon:(' . $icons_set . ')?:?([a-zA-Z0-9-_]+)/';

	return preg_replace_callback( $regex, 'buildex_render_icons_callback', $content );
}
 ...
 * @return string
 */
function buildex_render_icons_callback( $matches ) {
 ...
 * @return array
 */
function buildex_get_render_icons_set() {
	return apply_filters( 'buildex-theme/icons/icons-set', array(
 ...
 * @return string
 */
function buildex_render_theme_url( $url ) {
 ...
 * @return string
 */
function buildex_justify_thumbnail_size( $mask = 0, $thumbnail_size = 'post-thumbnail', $justify_size='buildex-thumb-justify', $justify_size_1 = 'buildex-thumb-justify', $justify_size_2 = 'buildex-thumb-justify-2') {
	$mask_list = array(
 ...
 * @return string
 */
function buildex_get_post_template_part_slug() {
	return apply_filters( 'buildex-theme/posts/template-part-slug', 'template-parts/content' );
}
 ...
 * @return string
 */
function buildex_get_post_style() {
	return apply_filters( 'buildex-theme/posts/post-style', false );
}
 ...
 * @return array
 */
function buildex_kses_post_allowed_html( $additional_allowed_html = array() ) {
