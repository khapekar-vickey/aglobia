 ...
 * @return array
 */
function buildex_related_posts() {

	if ( ! is_singular( 'post' ) ) {
		return;
	}

	$visible = buildex_theme()->customizer->get_value( 'related_posts_visible' );
	...
	$post_terms  = array();
	$post_number = buildex_theme()->customizer->get_value( 'related_posts_count' );
	...
	foreach ( $settings as $setting_key => $setting_value ) {
		$settings[ $setting_key ] = buildex_theme()->customizer->get_value( $setting_value );
	}
	...
	if ( $holder_view_dir ) {
		...
		foreach ( $posts as $post ) {
			...
			$image = ( $settings['image_visible'] ) ? buildex_post_thumbnail( 'buildex-thumb-s', array( 'echo' => false ) ) : '';
			...
			$author = ( $settings['author_visible'] ) ? buildex_posted_by( array( 'echo' => false ) ) : '';

			$date = ( $settings['date_visible'] ) ? buildex_posted_on( array( 'echo' => false ) ) : '';
