...
if ( ! class_exists( 'Buildex_Woo_Module' ) ) {

	/**
	 * Define Buildex_Woo_Module class
	 */
	class Buildex_Woo_Module extends Buildex_Module_Base {
		...
		public function filters() {
			...
			add_filter( 'buildex-theme/assets-depends/script', array( $this, 'assets_depends_script' ) );

			add_filter( 'buildex-theme/customizer/options', array( $this, 'customizer_options' ) );

			add_filter( 'cx_customizer/core_sections', array(  $this, 'buildex_customizer_core_sections' ) );
		}
		...
		*/
		...
		public function buildex_customizer_core_sections( $sections ) {
		...
		public function customizer_options( $options ) {
			...
			$new_options = array(
				'woocommerce_accent_color' => array(
					'title'    => esc_html__( 'WooCommerce Accent color', 'buildex' ),
				...
				'woocommerce_settings' => array(
					'title'           => esc_html__( 'WooCommerce Options', 'buildex' ),
					'priority'        => 200,
					'type'            => 'panel',
				),
				'single_product_layout' => array(
					'title'    => esc_html__( 'Single product layout', 'buildex' ),
					'panel'    => 'woocommerce_settings',
					'priority' => 1,
					'type'     => 'section',
				),
				'single_product_container_type' => array(
					'title'   => esc_html__( 'Single product container type', 'buildex' ),
		...
		public function assets_depends_script( $scripts_depends ) {
			array_push( $scripts_depends, 'buildex-woo-module-script' );
