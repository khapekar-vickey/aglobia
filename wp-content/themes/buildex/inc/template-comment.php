 ...
 */
function buildex_rewrite_comment_item( $_comment, $args, $depth ) {
	global $comment;

	$_comment->buildex_comment_list_args = $args;
	$comment = $_comment;
 ...
 */
function buildex_comment_author_avatar( $args = array() ) {
	global $comment;

	if ( ! empty( $comment->buildex_comment_list_args['avatar_size'] ) ) {
		$size = $comment->buildex_comment_list_args['avatar_size'];
	}
	 ...
	 * @param array $args   Arguments.
	 */
	return apply_filters( 'buildex-theme/comments/author-avatar', get_avatar( $comment, $size ), $args );
}
 ...
 */
function buildex_get_comment_author_link( $args = array() ) {
	/**
	 ...
	 * @param array $args   Arguments.
	 */
	return apply_filters( 'buildex-theme/comments/author-link', sprintf( '<b class="fn">%s</b>', get_comment_author_link() ), $args );
}
 ...
 */
function buildex_get_comment_date( $args = array() ) {
	$format = get_option( 'date_format' ) . ' ' . esc_attr_x( '\a\t', 'time prefix in comment date', 'buildex' ) . ' ' . get_option( 'time_format' );
	 ...
	 * @param array  $args   Arguments.
	 */
	return apply_filters( 'buildex-theme/comments/date', sprintf(
 ...
 */
function buildex_get_comment_text( $args = array() ) {
	 ...
	 * @param array  $args         Arguments.
	 */
	return apply_filters( 'buildex-theme/comments/text', $comment_text, $args );
}
 ...
 */
function buildex_get_comment_reply_link( $args = array() ) {
	global $comment_depth;
	 ...
	 * @param array  $args  Arguments.
	 */
	return apply_filters( 'buildex-theme/comments/reply_link', $reply, $args );
}
 ...
 */
function buildex_get_comment_link_edit( $args = array() ) {
	global $comment;

	$text = esc_html__( 'Edit', 'buildex' );
	 ...
	 * @param array  $args       Arguments.
	 */
	return apply_filters( 'buildex-theme/comments/link-edit', $link, $comment->comment_ID, $args );
}
